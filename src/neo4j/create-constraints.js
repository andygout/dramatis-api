...
const PROPERTIES_TO_CONSTRAIN = new Set([UUID_PROPERTY_NAME]);
...
const propertyNameToLabelSetMap = {
	[UUID_PROPERTY_NAME]: LABELS_REQUIRING_UUID_PROPERTY_CONSTRAINT
};
...
const createConstraintsOnProperty = async (existingConstraints, property) => {
	...
	const labelsMissingPropertyConstraint = [...propertyNameToLabelSetMap[property]].filter(
...
const createConstraints = async () => {
	...
	try {
		...
		for (const property of [...PROPERTIES_TO_CONSTRAIN]) {
