...
export default class SubVenue extends VenueBase {
	...
	async runDatabaseValidations({ subjectVenueUuid = null }) {
		if (this.name) {
			...
			const { isAssignedToSurVenue, isSurVenue, isSubjectVenueASubVenue } = await neo4jQuery({
			...
			if (isAssignedToSurVenue) {
				this.addPropertyError('name', 'Venue with these attributes is already assigned to another sur-venue');
