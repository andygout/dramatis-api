...
describe('Places controller', () => {
	...
	beforeEach(async () => {
		...
		placesController = await esmock('../../../src/controllers/places.js', {
	...
	const callFunction = async (functionName) => {
		return placesController[functionName](stubs.request, stubs.response, stubs.next);
	};
	...
	describe('newRoute function', () => {
		it('calls sendJsonResponse module', async () => {
			...
			sinonAssert.calledOnceWithExactly(
				...
				stubs.response,
				stubs.models.Place() // eslint-disable-line new-cap
			);
