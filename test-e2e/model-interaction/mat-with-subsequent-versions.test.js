...
const PEER_GYNT_SUBSEQUENT_VERSION_2_MATERIAL_UUID = 'PEER_GYNT_MATERIAL_3_UUID';
...
const GHOSTS_ORIGINAL_VERSION_MATERIAL_UUID = 'GHOSTS_MATERIAL_1_UUID';
const GHOSTS_SUBSEQUENT_VERSION_MATERIAL_UUID = 'GHOSTS_MATERIAL_2_UUID';
const PEER_GYNT_OLIVIER_PRODUCTION_UUID = 'PEER_GYNT_PRODUCTION_UUID';
const PEER_GYNT_BARBICAN_PRODUCTION_UUID = 'PEER_GYNT_PRODUCTION_2_UUID';
...
describe('Material with subsequent versions', () => {
	before(async () => {
		...
		peerGyntOriginalVersionMaterial = await request(app).get(
			`/materials/${PEER_GYNT_ORIGINAL_VERSION_MATERIAL_UUID}`
		);

		peerGyntSubsequentVersion2Material = await request(app).get(
			`/materials/${PEER_GYNT_SUBSEQUENT_VERSION_2_MATERIAL_UUID}`
		);

		henrikIbsenPerson = await request(app).get(`/people/${HENRIK_IBSEN_PERSON_UUID}`);

		gerryBammanPerson = await request(app).get(`/people/${GERRY_BAMMAN_PERSON_UUID}`);
		...
		peerGyntBarbicanProduction = await request(app).get(`/productions/${PEER_GYNT_BARBICAN_PRODUCTION_UUID}`);
	});
	...
	describe('Norway (place)', () => {
		it('includes in its material data the writers and (for subsequent versions) original version writers of the material', () => {
			...
			const { materials } = norwayPlace.body;
