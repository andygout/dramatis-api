...
const THE_WOLF_HALL_TRILOGY_NOVELS_MATERIAL_UUID = 'THE_WOLF_HALL_TRILOGY_MATERIAL_1_UUID';
...
let fifteenThirtyFiveTime;
let towerOfLondonPlace;
...
describe('Material with sub-materials and source materials thereof', () => {
	before(async () => {
		...
		fifteenThirtyFiveTime = await request(app).get(`/times/${FIFTEEN_THIRTY_FIVE_TIME_UUID}`);

		towerOfLondonPlace = await request(app).get(`/places/${TOWER_OF_LONDON_PLACE_UUID}`);

		prisonCellLocale = await request(app).get(`/locales/${PRISON_CELL_LOCALE_UUID}`);
	...
	describe('1535 (time)', () => {
		it('includes in its material data the writers of the material and its source material (with corresponding sur-material)', () => {
			...
			const { materials } = fifteenThirtyFiveTime.body;

			assert.deepEqual(materials, expectedMaterials);
		});
	});

	describe('Tower of London (place)', () => {
		it('includes in its material data the writers of the material and its source material (with corresponding sur-material)', () => {
			const expectedMaterials = [
				{
					model: 'MATERIAL',
			...
			const { materials } = towerOfLondonPlace.body;

			assert.deepEqual(materials, expectedMaterials);
		});
	});

	describe('Prison cell (locale)', () => {
		it('includes in its material data the writers of the material and its source material (with corresponding sur-material)', () => {
			const expectedMaterials = [
				{
					model: 'MATERIAL',
			...
			const { materials } = prisonCellLocale.body;
