	...
	describe('requests for instances that do not exist in database', () => {
		...
		describe('GET edit endpoint', () => {
			it('responds with 404 Not Found error', async () => {
				const response = await request.execute(app).get(`/seasons/${NON_EXISTENT_SEASON_UUID}/edit`);
		...
		describe('GET show endpoint', () => {
			it('responds with 404 Not Found error', async () => {
				const response = await request.execute(app).get(`/seasons/${NON_EXISTENT_SEASON_UUID}`);
		...
		describe('DELETE delete endpoint', () => {
			it('responds with 404 Not Found error', async () => {
				const response = await request.execute(app).delete(`/seasons/${NON_EXISTENT_SEASON_UUID}`);
